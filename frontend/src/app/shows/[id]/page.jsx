...
import {
  getShowDetails, getShowRecommendations,
  getShowFavorites, addShowFavorite, removeShowFavorite,
...
export default function ShowPage({ params }) {
  ...
  useEffect(() => {
    if (!user) return;
    getShowFavorites().then((favs) => setIsFav(favs.some((f) => f.tmdbShowId === Number(id))));
    getShowWatched().then((ws)     => setIsWatched(ws.some((w) => w.tmdbShowId === Number(id) && w.watched)));
  }, [user, id]);

  async function toggleFavorite() {
    if (isFav) { await removeShowFavorite(id); setIsFav(false); }
    else        { await addShowFavorite(id);    setIsFav(true); }
  }

  async function toggleWatched() {
    await setShowWatched(id, !isWatched);
    setIsWatched((w) => !w);
  }

  if (loading) return <Spinner />;
  if (!show)   return <p className="text-white/60 text-center py-20">Show not found.</p>;
  ...
  return (
    <div>
      ...
      <div className="relative h-[420px] w-full">
        {backdrop && (
          <Image src={backdrop} alt={show.name} fill className="object-cover opacity-30" priority />
        )}
        ...
        <div className="relative max-w-7xl mx-auto px-4 h-full flex items-end pb-8 gap-8">
          {poster && (
            <div className="hidden sm:block shrink-0">
              <Image src={poster} alt={show.name} width={180} height={270} className="rounded-lg shadow-2xl" />
            </div>
          )}
          ...
          <div className="text-white">
            ...
            <div className="flex flex-wrap gap-2 mt-2 text-sm text-white/60">
              ...
              {(seasons || episodes) && show.genres?.length > 0 && <span>·</span>}
              {show.genres?.map((g, i) => (
                <span key={g.id}>{g.name}{i < show.genres.length - 1 ? ',' : ''}</span>
              ))}
            </div>

            {creators && (
              <p className="text-white/50 text-sm mt-1">Created by {creators}</p>
            )}
              ...
              <div className="flex gap-3 mt-5">
                <button
                  ...
                  className={`px-4 py-2 rounded text-sm font-medium transition-colors ${
                    isFav ? 'bg-tmdb-accent text-white' : 'bg-white/10 text-white hover:bg-white/20'
                  }`}
                >
                ...
                <button
                  ...
                  className={`px-4 py-2 rounded text-sm font-medium transition-colors ${
                    isWatched ? 'bg-tmdb-green text-tmdb-darker' : 'bg-white/10 text-white hover:bg-white/20'
                  }`}
                >
      ...
      <div className="max-w-7xl mx-auto px-4 py-8 space-y-10">
        ...
        {/* Overview */}
        <section>
          {show.tagline && <p className="text-white/50 italic mb-3">"{show.tagline}"</p>}
              ...
              {cast.map((person) => {
                ...
                return (
                  <div key={person.id} className="bg-white/5 rounded-lg overflow-hidden">
                    {photo ? (
                      <Image src={photo} alt={person.name} width={185} height={278} className="w-full object-cover" />
                    ) : (
                      <div className="w-full aspect-[2/3] bg-white/10 flex items-center justify-center text-white/30 text-4xl">?</div>
                    )}
                    <div className="p-2">
